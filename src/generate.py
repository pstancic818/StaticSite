...
def generate_page(frompath, temppath, topath, basepath):
    ...
    tempfile = tempfile.replace(f'href="/', f'href="{basepath}')
    tempfile = tempfile.replace(f'src="/', f'src="{basepath}')
    if not os.path.exists(topath):
        os.makedirs(topath)
    with open(topath + '/index.html', 'w') as w:
        print(tempfile, file=w)
    
def generate_pages_recursive(content_path, temppath, topath, basepath):
    for i in os.listdir(content_path):
        if os.path.isdir(os.path.join(content_path, i)):
            generate_pages_recursive(os.path.join(content_path, i), temppath, os.path.join(topath, i), basepath)
        elif os.path.isfile(os.path.join(content_path, i)):
            generate_page(os.path.join(content_path, i), temppath, topath, basepath)
        else:
